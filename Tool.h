...
public:
  ...
  Tool(char const * pName,void (*pApplyFunction)(int,int,int,Tile * const));
  ...
  void apply(int x,int y,int colour,Tile * const tile) const;

  //the tool's name
  char const * name;

private:
  void (*applyFunction)(int,int,int,Tile * const);
};
